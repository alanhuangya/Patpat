...
import com.games.tap.config.UserConfig;
...
public class UserController {
    ...
    public Echo login(String username, String password) {
        ...
        // 将这个用户设置为全局用户
        UserConfig.globalUser = null;
        UserConfig.globalUser = saveUser;
    ...
    public Echo register(@RequestBody User user, HttpSession httpSession) {
        // 合法性校验
...
//        if (!Objects.equals(uid, user.getUId())) return Echo.fail("请求id不一致，id不可更改");
    ...
    public Echo getUserPostList(String uid, String offset, String pageSize, String order) {
        // 获取全局用户
        User user = UserConfig.globalUser;
        if (user == null) {
            return Echo.define(RetCode.USER_NOT_LOGGED_IN);
        } else {
